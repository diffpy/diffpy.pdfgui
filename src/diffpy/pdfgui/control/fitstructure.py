...
class FitStructure(PDFStructure):
    ...
    def insertAtoms(self, index, atomlist):
        ...
        for i in range(len(mag_insert)):
            mag_insert[i] = [0, ""]
        self.magnetic_atoms = self.magnetic_atoms[:index] + \
            mag_insert + self.magnetic_atoms[index:]
    ...
    def applyCMIPairSelection(self, pc):
        ...
        """
        psf = self.getPairSelectionFlags()
        # to begin masking, first remove ALL
        pc.setTypeMask("all", "all", False)
