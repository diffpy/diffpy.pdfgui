...
async def upload_file(
    ...
    if not file_service.validate_file(file.filename):
        raise HTTPException(
            status_code=400, detail=f"File type not allowed. Allowed: {settings.ALLOWED_EXTENSIONS}"
        )
    ...
    # Save file
    file_info = await file_service.save_upload(content, file.filename, str(current_user.id))

    # Parse file content
    try:
        parsed = file_service.parse_file(file_info["storage_path"], file_info["file_type"])
...
def list_files(
    file_type: str = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
...
def get_file(file_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Get file metadata."""
    uploaded_file = (
        db.query(UploadedFile).filter(UploadedFile.id == file_id, UploadedFile.user_id == current_user.id).first()
    )
...
def get_file_preview(file_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    preview = file_service.get_file_preview(uploaded_file.storage_path, uploaded_file.file_type)
...
async def delete_file(
    file_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
